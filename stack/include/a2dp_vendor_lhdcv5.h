...
// |retval| returns the translated quality mode from max target bit rate.
// Returns true if successfully
bool A2DP_VendorGetMaxBitRateLhdcV5(uint32_t *retval, const uint8_t* p_codec_info);

// Parse codec info and get the quality mode of the min target bit rate
// |p_codec_info_| contains the codec information.
// |retval| returns the translated quality mode from min target bit rate.
// Returns true if successfully
bool A2DP_VendorGetMinBitRateLhdcV5(uint32_t *retval, const uint8_t* p_codec_info);

// Get the LHDC sub version number from codec info
// |p_codec_info_| contains the codec information.
// |retval| returns the LHDC sub version number
// Returns true if successfully
bool A2DP_VendorGetVersionLhdcV5(uint32_t *retval, const uint8_t* p_codec_info);

// Get the bit per sample index from codec info
// |p_codec_info_| contains the codec information.
// |retval| returns the bit per sample index
// Returns true if successfully
bool A2DP_VendorGetBitPerSampleLhdcV5(uint8_t *retval, const uint8_t* p_codec_info);

// Get the LHDC Low latency mode flag from codec info
// |p_codec_info_| contains the codec information.
// |retval| returns the value of low latency mode flag
// Returns true if successfully
...
// Checks whether the codec capabilities contain a valid A2DP LHDC codec.
// NOTE: only codecs that are implemented are considered valid.
// Returns true if |p_codec_info| contains information about a valid LHDC codec,
// otherwise false.
bool A2DP_IsVendorSinkCodecValidLhdcV5(const uint8_t* p_codec_info);

// Checks whether the codec capabilities contain a valid peer A2DP LHDC Source
// codec.
// NOTE: only codecs that are implemented are considered valid.
// Returns true if |p_codec_info| contains information about a valid LHDC codec,
// otherwise false.
...
// Checks whether A2DP LHDC Sink codec is supported.
// |p_codec_info| contains information about the codec capabilities.
// Returns true if the A2DP LHDC Sink codec is supported, otherwise false.
bool A2DP_IsVendorSinkCodecSupportedLhdcV5(const uint8_t* p_codec_info);

// Checks whether an A2DP LHDC Source codec for a peer Source device is
// supported.
// |p_codec_info| contains information about the codec capabilities of the
// peer device.
// Returns true if the A2DP LHDC Source codec for a peer Source device is
...
// Gets the channel type for the A2DP LHDC Sink codec:
...
// Gets the A2DP LHDC decoder interface that can be used to decode received A2DP
// packets - see |tA2DP_DECODER_INTERFACE|.
// |p_codec_info| contains the codec information.
// Returns the A2DP LHDC decoder interface if the |p_codec_info| is valid and
// supported, otherwise NULL.
const tA2DP_DECODER_INTERFACE* A2DP_VendorGetDecoderInterfaceLhdcV5(
    const uint8_t* p_codec_info);

// Gets the A2DP LHDC Sink codec index for a given |p_codec_info|.
// Returns the corresponding |btav_a2dp_codec_index_t| on success,
// otherwise |BTAV_A2DP_CODEC_INDEX_MAX|.
btav_a2dp_codec_index_t A2DP_VendorSinkCodecIndexLhdcV5(const uint8_t* p_codec_info);

// Gets the A2DP LHDC V5 Sink codec name.
const char* A2DP_VendorCodecIndexStrLhdcV5Sink(void);

// Initializes A2DP LHDC Sink codec information into |AvdtpSepConfig|
// configuration entry pointed by |p_cfg|.
